...
colnames(wine_tmp) = c("Fix.A", "Vol.A", "Citric", "Sugar", "CI", 
                              "Free.SO2", "SO2", "Density", "pH", "log_SO4", "Alc", "quality")
...
ggplot(data= df, aes(x = alcohol, y = volatile.acidity, color = rating)) + 
  geom_point()
```
There seems to a relationship between alcohol percentage and wine quality. To examine the relationship better, I'll just plot 'bad' and 'good' red wine.

```{r echo=FALSE}
ggplot(data= subset(df, rating != "average"), aes(x = alcohol, y = volatile.acidity, color = rating)) + 
  geom_point()
```
...
```{r echo=FALSE}
ggplot(data= subset(df, rating != "average"), aes(x = alcohol, y = citric.acid, color = rating)) + 
  geom_point()
```
...
```{r echo=FALSE}
ggplot(data= subset(df, rating != "average"), aes(x = alcohol, y = log10(sulphates), color = rating)) + 
  geom_point()
```
...
```{r echo=FALSE}
ggplot(data= df, aes(x = volatile.acidity, y = citric.acid, color = rating)) + 
  geom_point()
```
...
```{r echo=FALSE}
ggplot(data= subset(df, rating != "average"), aes(x = volatile.acidity, y = log10(sulphates), color = rating)) + 
  geom_point()
```
...
```{r echo=FALSE}
ggplot(data= subset(df, rating != "average"), aes(x = citric.acid, y = log10(sulphates), color = rating)) + 
  geom_point()
```
...
The histogram of wine quality shows that the quality generally follows normal distribution. Most red wines are rated between 5 and 6.

### Plot Two: Effect of Alcohol levels on Red Wine Quality
```{r echo=FALSE, Plot_Two}
df$quality <- factor(df$quality, ordered = T)
ggplot(data = df, aes(x = quality, y = alcohol,fill = quality)) +
  geom_boxplot() +
...
These boxplots clearly demonstrate that wine qaulity is generally better with higher alcohol levels. At the meantime, alcohol alone does not produce a higher quality.
...
From the correlation study, we know that alcohol and volatile acidity have the highest correlation with quality. Plot 3 clearly supports this point. from the above graph, we can see that a combination of high alcohol levels and low volatile acidity produced better red wines.


## Reflection

From this exploratory data analysis, I was able to identity several main features which affect red wine quality: alcohol, volatile acidity and sulphates. alcohol has positive relationship with the quality of red wine while Volatile Acidity has negative one. Further analysis can be done by building a model to predict red wine quality by the 2 features: alcohol and volatile acidity.
